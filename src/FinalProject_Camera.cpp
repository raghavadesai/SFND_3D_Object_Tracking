                ...
                for(const auto& a_matcher_type : matcherTypeList )
                {
                    for(const auto& a_selector_type : selectorTypeList )
                    {
                        ...
                        for( int i=0 ; i < imgEndIndex; i++)
                        {
                            ...
                            newCombo.ttcLidar[i]  = 0.0f;
                            newCombo.ttcCamera[i] = 0.0f;
                        }
            ...
            if (dataBuffer.size() > 1) // wait until at least two images have been processed
            {
                ...
                for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
                {
                    ...
                    if( currBB->lidarPoints.size()>0 && prevBB->lidarPoints.size()>0 ) // only compute TTC if we have Lidar points
                    {
                        ...
                        /*RRD Changes Start */
                        a_combo.ttcLidar[imgIndex] = ttcLidar;
                        /*RRD Changes End */
                        //// EOF STUDENT ASSIGNMENT
    ...
    for( const auto& a_combo : Combinations )
    {
        for( int i= 0 ; i < imgEndIndex; i++)
        {
            outputFile << a_combo.detectorType
                       ...
                       << "," << std::fixed << std::setprecision(8) << a_combo.ttcLidar[i]
                       << "," << std::fixed << std::setprecision(8) << a_combo.ttcCamera[i]
                       << endl;
        }
        outputFile << endl;
    }
